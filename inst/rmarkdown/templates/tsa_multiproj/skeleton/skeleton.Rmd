...
## TODO delete aweek
## TODO move dat definers to control chunk
...
## use th models to get estimates and prediction intervals for each project
observed <- list()
...
for (i in unique(dat$project)) {
  ...
  observed[[paste0(i, "_predicted")]] <- predict(dat$fit[dat$project == i & 
                                                           dat$cut_off][[1]], 
                                                 dat$data[dat$project == i & 
                                                            !dat$cut_off][[1]])
...
#          ))
#              
# ## retrieve outputs in a single dataframe             
# observed <- dat %>% 
#   unnest(estimates)
...
pred_data <- pluck(dat, 1) %>% 
  select(total, epiweek, fourier, population) %>% 
  mutate(population = 1)
...
resid_dat <- nbreg %>% 
  ...
  mutate(origpred = purrr::map(.x = fit, ~ broom::augment(.x, 
