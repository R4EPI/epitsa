...
## TODO uncomment this section (shouldnt have to be installed in theory but whatever)
...
## TODO fix so that defining a z not necessary lower down?
...
##TODO delete static filepath (and uncomment below)

file_path <- paste0("C:\\Users\\Spina\\Desktop\\TSA_covid19\\Data\\Processed\\", z)
...
This chunk creates variables for fourier terms using the counts (or rates) data. 
The most appropriate weeks are selected by fourier series. These terms are then 
used for seasonality in the next chunk. 
For details on fourier series see links, as well as those in next chunk: 
...
This is based off work in the {trending} package: 
http://www.repidemicsconsortium.org/trending/index.html
...
## TODO: include a switch in control chunk to define method ("farrington" / "glrnb") 
  ## then use if statement to define - apply and store in object called algo_method
  ## then only need one line to incorporate in to observed dataframe (outside of if{})
...
for (i in projects) {
  ...
  # farringtonmethod <- farringtonFlexible(lrti_sts, ctrl)
  # 
...
for (i in projects) {
  ...
  #              )
  # ## apply farrington flexible method
  # farringtonmethod <- farringtonFlexible(deaths_sts, ctrl)
  # 
  # ## add farrington threshold to dataset
  # observed[which(observed$project == i &
  #                       observed$epiweek >= surveillance_date &
